...
} from '@chat-app-microservice/common';
...
export const initPublisher = async () => {
  if (!env.RABBITMQ_URL) {
    logger.warn('RABBITMQ_URL is not defined. Event publishing will not work.');
    return;
  }
...
export const closePublisher = async () => {
  try {
    ...
    logger.error({ error }, 'Error closing RabbitMQ publisher connection.');
  }
};
